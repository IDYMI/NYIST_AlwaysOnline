...
class LoginManager:
    ...
    def __init__(
        self,
        url_get_ip=HOST + "/cgi-bin/rad_user_info",
        url_get_challenge_api=HOST + "/cgi-bin/get_challenge",
        url_login_api=HOST + "/cgi-bin/srun_portal",
        n="200",
        vtype="1",
        acid="1",
        enc="srun_bx1",
    ...
    def _get_login_page(self):
        ...
        self._page_response = requests.get(
            self.url_get_ip, headers=header, params=params
        )
    ...
    def _resolve_ip_from_login_page(self):
        # 从字符串中提取JSON部分
        json_string = self._page_response.text.split("(", 1)[1].rsplit(")", 1)[0]

        # 解析JSON
        data = json.loads(json_string)
        # 提取client_ip
        self.ip = data.get("online_ip")
